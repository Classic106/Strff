    ...
    <div>
          ...
          <div class="card-body">
            ...
            <ul>
              <li
                ...
                :key="item.id"
                class="card-text mb-2 flex items-center justify-between"
              >
                <span
            ...
            <p v-if="cart.order_items && !cart.order_items.length">
      ...
      <div class="grid md:grid-cols-1 mt-3">
        <form autocomplete="off" @submit.stop.prevent="handleSubmit">
          ...
          <div class="mb-2">Card detail here</div>
          <button
            :disabled="loading"
            type="submit"
            class="
...
export default {
  ...
  computed: {
    async cart() {
      return await this.$store.getters["cart/cart"];
    },
    async numberOfItems() {
      return await this.$store.getters["cart/numberOfItems"];
    },
  },
  methods: {
    async handleSubmit() {
    ...
    addToCart(item) {
      let selected = {
        ...
        subscriptionTypeId: item.subscription_type
          ? item.subscription_type.id
          : null,
      };
