...
module.exports = {
  async addProduct(ctx) {
    ...
    const orderPendingStatus = await strapi.services["order-statuses"].findOne({
      code: 1,
    });
    ...
    if (params.purchaseTypeId) {
      purchaseType = await strapi.services["purchase-type"].findOne({
    ...
    if (params.userId) {
      options = {
        "order_status.id": orderPendingStatus.id,
    ...
    if (order.id) {
      ...
      if (subscriptionType) {
        options["subscription_type.id"] = subscriptionType
          ? subscriptionType.id
          : null;
      }
      let orderItem = null;
      ...
      if (!orderItems.length) {
        ...
        orderItem = await strapi.services["order-item"].update(
          { id: orderItem.id },
          orderItem
        );
      }
  ...
  async updateItem(ctx) {
    ...
    let orderItem = await strapi.services["order-item"].findOne({
      id: params.id,
    });
    if (orderItem) {
      orderItem.quantity = params.quantity;
  ...
  async addBundle(ctx) {
    ...
    const bundle = await strapi.services.bundle.findOne({
      id: params.bundleId,
    });
    const orderPendingStatus = await strapi.services["order-statuses"].findOne({
      code: 1,
  ...
  async recomputeOrder(id) {
    ...
    if (order) {
      ...
      let orderBundles = await strapi.services["order-bundle"].find({
        "order.id": order.id,
      });
      if (orderBundles) {
